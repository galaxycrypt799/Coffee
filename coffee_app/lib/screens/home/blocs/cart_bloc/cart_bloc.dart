...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  Future<void> _onUpdateQuantity(UpdateQuantityEvent event, Emitter<CartState> emit) async {
    try {
      final currentCart = _getCurrentCart();
      final updatedCart = currentCart.updateQuantity(event.itemId, event.quantity);
      await _persistCart(updatedCart);
      emit(CartUpdated(updatedCart));
    } catch (e) {
  ...
  Future<void> _onLoadCart(LoadCartEvent event, Emitter<CartState> emit) async {
    try {
      ...
      if (cartJson != null) {
        final cart = Cart.fromJson(jsonDecode(cartJson) as Map<String, dynamic>);
